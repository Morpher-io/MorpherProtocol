...
describe('MorpherBridge: change Limits tests', () => {
    ...
    it('is possible to change 24hours limits', async() => {
        ...
        let currentLimit = await morpherBridge.withdrawalLimitPerUserDaily();
        assert.equal(currentLimit.toString(), web3.utils.toWei('1','ether'));

        //set it back
        await morpherBridge.updateWithdrawLimitPerUserDaily(web3.utils.toWei('200000', 'ether'));
        await morpherAccessControl.revokeRole(await morpherBridge.SIDECHAINOPERATOR_ROLE(), accounts[0]);
    });
    ...
    it('is possible to change 30 days limits', async() => {
        ...
        await morpherAccessControl.grantRole(await morpherBridge.SIDECHAINOPERATOR_ROLE(), accounts[0]);        
        let result = await morpherBridge.updateWithdrawLimitPerUserMonthly(web3.utils.toWei('1', 'ether'));
        await truffleAssert.eventEmitted(result, 'WithdrawLimitMonthlyPerUserChanged');
        let currentLimit = await morpherBridge.withdrawalLimitPerUserMonthly();
        assert.equal(currentLimit.toString(), web3.utils.toWei('1','ether'));

        //set it back
        await morpherBridge.updateWithdrawLimitPerUserMonthly(web3.utils.toWei('1000000', 'ether'));
        await morpherAccessControl.revokeRole(await morpherBridge.SIDECHAINOPERATOR_ROLE(), accounts[0]);
    ...
    it('is possible to change 365 days limits', async() => {
        ...
        const morpherBridge = await MorpherBridge.deployed();
        await morpherAccessControl.grantRole(await morpherBridge.SIDECHAINOPERATOR_ROLE(), accounts[0]);        
        let result = await morpherBridge.updateWithdrawLimitPerUserYearly(web3.utils.toWei('1', 'ether'));
        await truffleAssert.eventEmitted(result, 'WithdrawLimitYearlyPerUserChanged');
        let currentLimit = await morpherBridge.withdrawalLimitPerUserYearly();
        assert.equal(currentLimit.toString(), web3.utils.toWei('1','ether'));

        //set it back
        await morpherBridge.updateWithdrawLimitPerUserYearly(web3.utils.toWei('5000000', 'ether'));      
        await morpherAccessControl.revokeRole(await morpherBridge.SIDECHAINOPERATOR_ROLE(), accounts[0]);
...
contract('MorpherBridge: transferToSidechain tests', (accounts) => {
    ...
    it('test withdrawal transferToSideChain daily limit', async () => {
        ...
        await morpherBridge.updateWithdrawLimitPerUserDaily(web3.utils.toWei("20", "ether"));
        await morpherAccessControl.grantRole(await morpherToken.TRANSFER_ROLE(), deployer);
        ...
        let result = await morpherBridge.stageTokensForTransfer(web3.utils.toWei("20", "ether"), { from: addr1 });
        ...
        await truffleAssert.fails(
                    morpherBridge.stageTokensForTransfer(web3.utils.toWei("1", "ether"), { from: addr1 }),
        ...
        await morpherAccessControl.grantRole(await morpherToken.TRANSFER_ROLE(), addr1);

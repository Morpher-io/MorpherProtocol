...
contract('MorpherTradeEngine', (accounts) => {
    ...
    it('test case 1: rollover 10 longshares to 5 short shares with spread 5x', async () => {
        ...
        console.log((await morpherTradeEngine.getNow()).toString(), Math.round(Date.now() / 1000), createdTimestamp, (await morpherTradeEngine.calculateMarginInterest(roundToInteger(300), 200000000, createdTimestamp)).toString());
        ...
        console.log(position._shortShares.toString(), position._meanEntryLeverage.toString(), position._meanEntryPrice.toString(), position._meanEntrySpread.toString());
        ...
        assert.equal(positionValue, 79980000000);
        ...
        assert.equal(userBalance, '1000000000019946050000');
    });
});
...
//         assert.equal(positionValue, 9977505000000);
...
//         assert.equal(userBalance, '999999996017205000000');
//     });
// });

...
contract MorpherAccessControlTest is Test {
    ...
    function setUp() public {
        ...
        transparentProxyState = new TransparentUpgradeableProxy(
        ...
        transparentProxyAccessControl.changeAdmin(address(proxyAdmin));
        transparentProxyState.changeAdmin(address(proxyAdmin));

        MorpherAccessControl(address(transparentProxyAccessControl))
            .initialize();
        MorpherState(address(transparentProxyState)).initialize(
            false,
            address(transparentProxyAccessControl)
        );

        proxiedAccessControl = MorpherAccessControl(
            address(transparentProxyAccessControl)
        );
    ...
    function testHasRole() public {
        assertEq(
            proxiedAccessControl.hasRole(
                proxiedState.ADMINISTRATOR_ROLE(),
                address(this)
            ), false
        );

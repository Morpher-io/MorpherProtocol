...
//         UUPSUpgradeable(state.morpherOracleAddress()).upgradeTo(_oracleAddress);
//     }

//     function setAdministrator(address _administratorAddress) private {
//         MorpherAccessControl(state.morpherAccessControlAddress()).grantRole(ADMINISTRATOR_ROLE, _administratorAddress);
//     }

//     function getMorpherOracle() public view returns (address _address)  {
//         return state.morpherOracleAddress();
//     }
...
//         require(MorpherToken(state.morpherTokenAddress()).balanceOf(msg.sender) >= _requiredAmount, "MorpherGovernance: Insufficient balance to become Validator.");
...
//         MorpherToken(state.morpherTokenAddress()).burn(msg.sender, _requiredAmount);
...
//         require(MorpherToken(state.morpherTokenAddress()).balanceOf(address(this)) >= MINVALIDATORLOCKUP * (numberOfValidators), "MorpherGovernance: Escrow does not have enough funds. Should not happen.");
...
//             MorpherToken(state.morpherTokenAddress()).mint(validatorAddress[i], MINVALIDATORLOCKUP / 100 * (99));
...
//         MorpherToken(state.morpherTokenAddress()).mint(msg.sender, MINVALIDATORLOCKUP * (_myValidatorIndex) / 100 * (99));

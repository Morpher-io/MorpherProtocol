...
import "../contracts/MorpherBridgeFixMainchain.sol";
import "../contracts/MorpherState.sol";
import "../contracts/MorpherAccessControl.sol";
import "../contracts/MorpherToken.sol";
import "../contracts/MorpherDeprecatedTokenMapper.sol";
import "../contracts/interfaces/IMorpherStateDeprecated.sol";
...
contract MorpherBridgeTest is Test, MorpherBridge {
	...
	function deployNewTokensMapper() public {
		address mainchain_owner = address(0x51c5cE7C4926D5cA74f4824e11a062f1Ef491762);
		vm.startPrank(mainchain_owner);
		MorpherDeprecatedTokenMapper mapper = new MorpherDeprecatedTokenMapper();
		morpherState.setMorpherToken(address(mapper));
		IMorpherStateDeprecated oldState = IMorpherStateDeprecated(0x1f426C51F0Ef7655A6f4c3Eb58017d2F1c381bfF);
		mapper.initialize(address(morpherState));
		mapper.updateDeprecatedMorpherStateAddress(0x1f426C51F0Ef7655A6f4c3Eb58017d2F1c381bfF);
		oldState.grantAccess(address(mapper));
		vm.stopPrank();
	}
	function testClaimTokensNewMapperMainchain() public {
		// address mainchain_owner = address(0x51c5cE7C4926D5cA74f4824e11a062f1Ef491762);
		address callback_addr1 = address(0xCCCcc0405E2eA54Ca901f929965227c50235D87b);
		deployNewTokensMapper();
		MorpherBridge morpherBridge = MorpherBridge(payable(morpherState.morpherBridgeAddress()));
	...
	function deployNewBridge() public {
		address mainchain_owner = address(0x51c5cE7C4926D5cA74f4824e11a062f1Ef491762);

		ISwapRouter swapRouter_baseSetup = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);

		vm.startPrank(mainchain_owner);
		MorpherBridgeFixMainchain newBridge = new MorpherBridgeFixMainchain();

		TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(address(newBridge), address(this), "");
		// ProxyAdmin proxyAdmin = new ProxyAdmin();
		// proxy.changeAdmin(address(proxyAdmin));

		MorpherBridge(address(proxy)).initialize(address(morpherState), false, swapRouter_baseSetup);

		IMorpherStateDeprecated oldState = IMorpherStateDeprecated(0x1f426C51F0Ef7655A6f4c3Eb58017d2F1c381bfF);

		oldState.grantAccess(address(proxy));

		morpherState.setMorpherBridge(address(proxy));

		morpherState.setMorpherToken(0x1f426C51F0Ef7655A6f4c3Eb58017d2F1c381bfF);
		vm.stopPrank();
	}

	function testClaimTokensNewBridgeMainchain() public {
		address callback_addr1 = address(0xCCCcc0405E2eA54Ca901f929965227c50235D87b);
		deployNewBridge();
	...
	function testClaimOnlyTokensMiniProofMainchain() public {
		...
		uint oldBalance = oldMorpherToken.balanceOf(usrAddr);
		vm.prank(usrAddr);
		vm.chainId(1);
		//Function: claimStagedTokensAndSendForUser(address _usrAddr, uint256 _numOfToken, uint256 fee, address feeRecipient, uint256 _claimLimit, bytes32[] _proof, address _finalOutput, bytes32 _rootHash, bytes _userConfirmationSignature)
		morpherBridge.claimStagedTokens(numTokens, claimLimit, proof);

		assertEq(oldMorpherToken.balanceOf(usrAddr), oldBalance + numTokens);
	}
}

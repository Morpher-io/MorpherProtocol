...
contract MorkpherTradingEngineTest is BaseSetup {
	...
	function _openPositionForMintingLimiterTest(
		address user
	) internal returns (OpenPositionInfo memory info) {
		// Open Position
		...
		(
			...
			,
			info.meanEntryPrice,
			info.meanEntrySpread,
			info.meanEntryLeverage,
			,
	...
	function testMintedTokenTransferLimit() public {
		...
		address user = address(0xff01);
		address recipient = address(0xff02);
		
		// Set up a position and close it to generate profit
		uint256 marketPrice = 50000 * PRECISION;
		uint256 marketSpread = 10 * PRECISION;
		uint256 orderLeverage = 5 * PRECISION;
		...
		uint256 expectedShareValue = (50050 * PRECISION) *
			5 -
			50000 *
			PRECISION *
			(5 - 1) -
	...
	function testProfitSentToMintingLimiterAndPrincipalMintedDirectly() public {
		...
		// 2. Open Position
		OpenPositionInfo memory posInfo = _openPositionForMintingLimiterTest(user);
